...
class View(tkinter.Tk):
    def __init__(self, controller):
        ...
        modify_menu.add_command(label="Add Case", command=self.controller.add_case)
...
class Container_Ticket_Bar(tkinter.Frame):
    def __init__(self, parent, controller):
        ...
        newticket_button = tkinter.Button(self, text="New Ticket", width=10, command=self.controller.handle_new_ticket_clicked)
    ...
    def refresh_ticket_section(self, all_tickets):
        for i, ticket in enumerate(all_tickets):
            if i < 7:
                frame = tkinter.Frame(self, width=75, height=75, borderwidth=1, relief="solid")
                frame.pack_propagate(False)
                l_name = tkinter.Label(frame, text=ticket.name)
                l_name.bind("<Button-1>", self.controller.handle_ticket_clicked)
...
class Container_Ticket_Details(tkinter.Frame):
    def __init__(self, parent, controller):
        ...
        for i, component in enumerate(self.general_components):
            ...
            if i > 0:
                entry.bind("<Up>", lambda event, i=i: self.general_components[i-1][1].focus_set())

            # Bind DOWN key to move down the entry list as long as it's not the last entry.
            if i < len(self.general_components) - 1:
                entry.bind("<Down>", lambda event, i=i: self.general_components[i+1][1].focus_set())
        ...
        for i, component in enumerate(self.computer_components):
            ...
            # Bind UP key to move up the entry list as long as it's not the first entry
            if i > 0:
                entry.bind("<Up>", lambda event, i=i: self.computer_components[i-1][1].focus_set())
                replace_entry.bind("<Up>", lambda event, i=i: self.computer_components[i-1][3].focus_set())
                
            # Bind DOWN key to move down the entry list as long as it's not the last entry.
            if i < len(self.computer_components) - 1:
                entry.bind("<Down>", lambda event, i=i: self.computer_components[i+1][1].focus_set())
                replace_entry.bind("<Down>", lambda event, i=i: self.computer_components[i+1][3].focus_set())
